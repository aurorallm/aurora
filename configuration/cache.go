...
type LocalCacheConfig struct {
	CacheDir string `yaml:"cache_dir" env:"AURORA_CACHE_DIR"`
}

// ModelListConfig holds configuration for the external model metadata registry.
//
// Resolution order on startup:
//  1. LocalPath (if file exists) — primary source of truth, vendored in-repo
//  2. URL fetch (if LocalPath missing or empty) — backward-compatible fallback
//
// User edits to the registry should go in UserOverridesPath (a small YAML file
// of per-field overrides) rather than mutating LocalPath, which is regenerated
// by `aurora models sync`.
type ModelListConfig struct {
	// URL is the HTTP(S) URL to fetch models.json from. Used by `aurora models
	// sync` to regenerate LocalPath, and as a startup fallback when LocalPath
	// does not exist. Empty disables both.
	URL string `yaml:"url" env:"MODEL_LIST_URL"`

	// LocalPath is the on-disk JSON snapshot of the model registry. Loaded
	// synchronously at startup when present; missing means "fall back to URL".
	// Default: data/models.local.json
	LocalPath string `yaml:"local_path" env:"MODEL_LIST_LOCAL_PATH"`

	// UserOverridesPath is a YAML file of per-field overrides applied on top
	// of the loaded model list. Operator values win per-field. Empty / missing
	// file is a no-op. Default: data/user_pricing.yaml
	UserOverridesPath string `yaml:"user_overrides_path" env:"MODEL_LIST_USER_OVERRIDES_PATH"`
}
...
type SemanticCacheConfig struct {
	...
	PromptSimilarityThreshold float64           `yaml:"prompt_similarity_threshold"`
	TTL                       *int              `yaml:"ttl"`
...
func ValidateCacheConfig(c *CacheConfig) error {
	...
	if sem != nil && SemanticCacheActive(sem) {
		...
		pt := sem.PromptSimilarityThreshold
		if math.IsNaN(pt) || math.IsInf(pt, 0) || pt < 0 || pt > 1 {
			return fmt.Errorf("cache.response.semantic.prompt_similarity_threshold: must be between 0 and 1 (yaml: prompt_similarity_threshold, env: SEMANTIC_CACHE_PROMPT_SIMILARITY); got %v", pt)
		}
...
func mergeSemanticResponseDefaults(sem *SemanticCacheConfig) {
	...
	if sem.PromptSimilarityThreshold == 0 {
		sem.PromptSimilarityThreshold = 0.72
	}
	if sem.TTL == nil {
		sem.TTL = intPtr(3600)
	}
...
func applyResponseSemanticEnv(resp *ResponseCacheConfig) error {
	...
	if val := os.Getenv("SEMANTIC_CACHE_PROMPT_SIMILARITY"); val != "" {
		f, err := strconv.ParseFloat(val, 64)
		if err != nil {
			return fmt.Errorf("invalid value for SEMANTIC_CACHE_PROMPT_SIMILARITY: %q is not a valid float", val)
		}
		sem.PromptSimilarityThreshold = f
	}
