...
func (p *ContentPart) CacheControl() CacheControl {
	cc, _ := UnmarshalCacheControlFromExtra(p.ExtraFields)
	return cc
}

func (p *ContentPart) SetCacheControl(cc CacheControl) {
	p.ExtraFields = MarshalCacheControlToExtra(p.ExtraFields, cc)
}
...
// UnmarshalCacheControlFromExtra extracts a CacheControl from UnknownJSONFields.
// Returns the CacheControl and true if a valid cache_control entry was found.
func UnmarshalCacheControlFromExtra(extra UnknownJSONFields) (CacheControl, bool) {
	if extra.IsEmpty() {
		return CacheControl{}, false
	}
	raw := extra.Lookup("cache_control")
	if len(raw) == 0 {
		return CacheControl{}, false
	}
	var cc struct {
	...
	extraFields, err := extractUnknownJSONFields(raw, "type")
	if err != nil {
		return CacheControl{}, false
	}
	return CacheControl{Type: cc.Type, ExtraFields: extraFields}, true
}

// MarshalCacheControlToExtra injects cache_control into UnknownJSONFields.
// If cc is empty, any existing cache_control is removed from extra.
func MarshalCacheControlToExtra(extra UnknownJSONFields, cc CacheControl) UnknownJSONFields {
	if cc.IsEmpty() {
		return removeJSONField(extra, "cache_control")
	}
	ccData, err := json.Marshal(cc)
	if err != nil {
		return extra
	}
	return mergeJSONField(extra, "cache_control", ccData)
}

func removeJSONField(extra UnknownJSONFields, field string) UnknownJSONFields {
	if extra.IsEmpty() {
		return extra
	}
	var rawMap map[string]json.RawMessage
	if err := json.Unmarshal(extra.raw, &rawMap); err != nil {
		return extra
	}
	delete(rawMap, field)
	if len(rawMap) == 0 {
	...
	return UnknownJSONFieldsFromMap(rawMap)
}

func mergeJSONField(extra UnknownJSONFields, field string, value json.RawMessage) UnknownJSONFields {
	var rawMap map[string]json.RawMessage
	if !extra.IsEmpty() {
		_ = json.Unmarshal(extra.raw, &rawMap)
	}
	if rawMap == nil {
