...
const (
	...
	// ErrorTypeAuthorization indicates a permission denied error (403)
...
// NewAuthorizationError creates a new authorization error (403)
func NewAuthorizationError(provider string, message string) *GatewayError {
	return &GatewayError{
...
func ParseProviderError(provider string, statusCode int, body []byte, originalErr error) *GatewayError {
	...
	case statusCode >= 400 && statusCode < 500:
		...
		gatewayErr = NewInvalidRequestErrorWithStatus(statusCode, providerErrorMessage(provider, statusCode, message), originalErr)
		gatewayErr.Provider = provider
	case statusCode >= 500:
...
func providerErrorMessage(provider string, statusCode int, message string) string {
	...
	if message == "" {
		return fmt.Sprintf("upstream provider %s returned HTTP %d", provider, statusCode)
	}
	return fmt.Sprintf("upstream provider %s returned HTTP %d: %s", provider, statusCode, message)
}
