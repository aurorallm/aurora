...
// CacheControl returns the message-level cache control directive, or nil
// if none is set. This works via ExtraFields round-trip.
func (m *Message) CacheControl() *CacheControl {
	if m == nil {
		return nil
	}
	cc, ok := UnmarshalCacheControlFromExtra(m.ExtraFields)
	if !ok {
		return nil
	}
	return &cc
...
// SetCacheControl sets the message-level cache control directive via
// ExtraFields so it survives JSON round-trips.
func (m *Message) SetCacheControl(cc *CacheControl) {
	if m == nil {
		return
	}
	if cc == nil {
		m.ExtraFields = MarshalCacheControlToExtra(m.ExtraFields, CacheControl{})
		return
	}
	m.ExtraFields = MarshalCacheControlToExtra(m.ExtraFields, *cc)
}
