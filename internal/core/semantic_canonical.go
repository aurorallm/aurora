...
var canonicalOperationCodecs = map[Operation]canonicalOperationCodec{
	...
	OperationRerank: newCanonicalOperationCodec(semanticRerankRequestKey, func() *RerankRequest { return &RerankRequest{} }, func(env *WhiteBoxPrompt, req *RerankRequest) {
		cacheSemanticSelectorHintsFromRequest(env, req)
	}),
...
// DecodeRerankRequest decodes and caches the canonical rerank request for a semantic envelope.
func DecodeRerankRequest(body []byte, env *WhiteBoxPrompt) (*RerankRequest, error) {
	return decodeCanonicalOperation[*RerankRequest](body, env, OperationRerank)
}
