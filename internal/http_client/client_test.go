...
func TestDefaultConfig(t *testing.T) {
	...
	if config.MaxIdleConns != 4096 {
		t.Errorf("Expected MaxIdleConns to be 4096, got %d", config.MaxIdleConns)
	}

	if config.MaxIdleConnsPerHost != 4096 {
		t.Errorf("Expected MaxIdleConnsPerHost to be 4096, got %d", config.MaxIdleConnsPerHost)
	}
	if config.MaxConnsPerHost != 0 {
		t.Errorf("Expected MaxConnsPerHost to be 0, got %d", config.MaxConnsPerHost)
	}
...
func TestDefaultConfigWithEnvOverrides(t *testing.T) {
	...
	t.Setenv("HTTP_MAX_IDLE_CONNS", "2048")
	t.Setenv("HTTP_MAX_IDLE_CONNS_PER_HOST", "1024")
	t.Setenv("HTTP_MAX_CONNS_PER_HOST", "512")
	...
	if config.MaxIdleConns != 2048 {
		t.Errorf("Expected MaxIdleConns to be 2048 from env, got %d", config.MaxIdleConns)
	}

	if config.MaxIdleConnsPerHost != 1024 {
		t.Errorf("Expected MaxIdleConnsPerHost to be 1024 from env, got %d", config.MaxIdleConnsPerHost)
	}
	if config.MaxConnsPerHost != 512 {
		t.Errorf("Expected MaxConnsPerHost to be 512 from env, got %d", config.MaxConnsPerHost)
	}
...
func TestNewHTTPClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if transport.MaxConnsPerHost != expectedConfig.MaxConnsPerHost {
				t.Errorf("Expected MaxConnsPerHost to be %d, got %d", expectedConfig.MaxConnsPerHost, transport.MaxConnsPerHost)
			}
