...
func TestStreamChatCompletion_ThinkingBlockEmitsOpenAIReasoningAndThinkMarkersLike9router(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`event: message_start
data: {"type":"message_start","message":{"id":"msg_123","type":"message","role":"assistant","model":"claude-sonnet-4-5-20250929","content":[],"stop_reason":null,"usage":{"input_tokens":10,"output_tokens":0}}}
...
`))
	...
	responseStr := string(raw)
	for _, want := range []string{"\"content\":\"\\u003cthink\\u003e\"", `"reasoning_content":"reasoning"`, "\"content\":\"\\u003c/think\\u003e\"", `"content":"answer"`} {
		if !strings.Contains(responseStr, want) {
			t.Fatalf("stream missing %s: %s", want, responseStr)
		}
	}
}
...
func TestConvertToAnthropicRequest_InjectsJSONResponseFormatSystemPromptLike9router(t *testing.T) {
	req := &core.ChatRequest{
		Model: "claude-sonnet-4-5-20250929",
		Messages: []core.Message{
			{Role: "system", Content: "Follow policy."},
			{Role: "user", Content: "return coordinates"},
		},
		ExtraFields: core.UnknownJSONFieldsFromMap(map[string]json.RawMessage{
			"response_format": json.RawMessage(`{"type":"json_schema","json_schema":{"schema":{"type":"object","properties":{"lat":{"type":"number"}},"required":["lat"]}}}`),
		}),
	}
	...
	if !ok {
		t.Fatalf("System = %T, want string", result.System)
	}
	for _, want := range []string{
		...
		if !strings.Contains(system, want) {
			t.Fatalf("System missing %q: %s", want, system)
		}
	}
}

func TestConvertToAnthropicRequest_InjectsJSONObjectResponseFormatSystemPromptLike9router(t *testing.T) {
	req := &core.ChatRequest{
		Model: "claude-sonnet-4-5-20250929",
		Messages: []core.Message{
	...
	if result.System != "You must respond with valid JSON. Respond ONLY with a JSON object, no other text." {
		t.Fatalf("System = %#v, want JSON object instruction", result.System)
	}
}

func TestConvertToAnthropicRequest_MergesConsecutiveOpenAIMessagesLike9router(t *testing.T) {
	req := &core.ChatRequest{
		Model: "claude-sonnet-4-5-20250929",
		Messages: []core.Message{
	...
	if len(result.Messages) != 2 {
		t.Fatalf("len(Messages) = %d, want 2 merged role turns: %#v", len(result.Messages), result.Messages)
	}

	if result.Messages[0].Role != "user" {
		t.Fatalf("Messages[0].Role = %q, want user", result.Messages[0].Role)
	}
	userBlocks, ok := result.Messages[0].Content.([]anthropicContentBlock)
	if !ok {
		t.Fatalf("Messages[0].Content = %T, want []anthropicContentBlock", result.Messages[0].Content)
	}
	if len(userBlocks) != 2 || userBlocks[0].Text != "first user" || userBlocks[1].Text != "second user" {
		t.Fatalf("unexpected merged user blocks: %+v", userBlocks)
	}

	if result.Messages[1].Role != "assistant" {
		t.Fatalf("Messages[1].Role = %q, want assistant", result.Messages[1].Role)
	}
	assistantBlocks, ok := result.Messages[1].Content.([]anthropicContentBlock)
	if !ok {
		t.Fatalf("Messages[1].Content = %T, want []anthropicContentBlock", result.Messages[1].Content)
	}
	if len(assistantBlocks) != 2 || assistantBlocks[0].Text != "assistant reply" || assistantBlocks[1].Text != "assistant follow-up" {
		t.Fatalf("unexpected merged assistant blocks: %+v", assistantBlocks)
	}
}

func TestConvertToAnthropicRequest_SeparatesToolResultsLike9router(t *testing.T) {
	req := &core.ChatRequest{
		Model: "claude-sonnet-4-5-20250929",
		Messages: []core.Message{
			{
	...
	if len(result.Messages) != 3 {
		t.Fatalf("len(Messages) = %d, want assistant/tool_result/user turns: %#v", len(result.Messages), result.Messages)
	}

	assistantBlocks, ok := result.Messages[0].Content.([]anthropicContentBlock)
	if !ok || len(assistantBlocks) != 2 {
		t.Fatalf("assistant content = %#v, want text + tool_use blocks", result.Messages[0].Content)
	}
	if assistantBlocks[0].Type != "text" || assistantBlocks[1].Type != "tool_use" {
		t.Fatalf("unexpected assistant blocks: %+v", assistantBlocks)
	}

	toolBlocks, ok := result.Messages[1].Content.([]anthropicContentBlock)
	if !ok || len(toolBlocks) != 1 {
		t.Fatalf("tool result content = %#v, want one tool_result block", result.Messages[1].Content)
	}
	if result.Messages[1].Role != "user" || toolBlocks[0].Type != "tool_result" || toolBlocks[0].ToolUseID != "call_weather" {
		t.Fatalf("unexpected tool result turn: role=%q blocks=%+v", result.Messages[1].Role, toolBlocks)
	}

	if result.Messages[2].Content != "now summarize" {
		t.Fatalf("unexpected final user turn: %#v", result.Messages[2].Content)
	}
}
