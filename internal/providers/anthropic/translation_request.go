...
// aurora injects this value to keep the OpenAI-compatible surface lenient.
...
func convertToAnthropicRequest(req *core.ChatRequest) (*anthropicRequest, error) {
	...
	currentBlocks := make([]anthropicContentBlock, 0)
	flushCurrentMessage := func() {
		...
		anthropicReq.Messages = append(anthropicReq.Messages, anthropicMessage{
			Role:    currentRole,
			Content: anthropicBlocksContent(currentBlocks),
		})
		currentRole = ""
	...
	for _, msg := range req.Messages {
		...
		hasToolUse := anthropicBlocksContainType(blocks, "tool_use")
		hasToolResult := anthropicBlocksContainType(blocks, "tool_result")

		if hasToolResult {
			toolResultBlocks := filterAnthropicBlocksByType(blocks, "tool_result", true)
			otherBlocks := filterAnthropicBlocksByType(blocks, "tool_result", false)
			flushCurrentMessage()
			...
			if len(otherBlocks) > 0 {
				currentRole = role
				currentBlocks = append(currentBlocks, otherBlocks...)
			}
			continue
		}
	...
	flushCurrentMessage()
	anthropicReq.System = appendAnthropicSystemContent(anthropicReq.System, responseFormatInstruction)
...
func anthropicResponseFormatInstruction(extraFields core.UnknownJSONFields) (any, error) {
	...
	case "json_schema":
		if responseFormat.JSONSchema == nil || len(bytes.TrimSpace(responseFormat.JSONSchema.Schema)) == 0 {
			return nil, nil
		}
		var pretty bytes.Buffer
		if err := json.Indent(&pretty, responseFormat.JSONSchema.Schema, "", "  "); err != nil {
			return nil, core.NewInvalidRequestError("response_format.json_schema.schema must be valid JSON", err)
		}
		return "You must respond with valid JSON that strictly follows this JSON schema:\n```json\n" + pretty.String() + "\n```\nRespond ONLY with the JSON object, no other text.", nil
	case "json_object":
		return "You must respond with valid JSON. Respond ONLY with a JSON object, no other text.", nil
	default:
		return nil, nil
	}
}
...
func anthropicContentBlocks(content any) []anthropicContentBlock {
	...
	case []anthropicContentBlock:
		return append([]anthropicContentBlock(nil), c...)
	default:
		return nil
	}
}

func anthropicBlocksContent(blocks []anthropicContentBlock) any {
	if len(blocks) == 1 && blocks[0].Type == "text" && len(blocks[0].CacheControl) == 0 {
		return blocks[0].Text
	}
	return append([]anthropicContentBlock(nil), blocks...)
}

func anthropicBlocksContainType(blocks []anthropicContentBlock, blockType string) bool {
...
func filterAnthropicBlocksByType(blocks []anthropicContentBlock, blockType string, include bool) []anthropicContentBlock {
	out := make([]anthropicContentBlock, 0, len(blocks))
	for _, block := range blocks {
		if (block.Type == blockType) == include {
