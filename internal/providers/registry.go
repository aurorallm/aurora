...
// SetUserOverridesPath records the path to a YAML file of per-field model
// overrides. The file is re-read on every model-list load/refresh so users can
// edit it without a process restart. Empty disables user overrides.
func (r *ModelRegistry) SetUserOverridesPath(path string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.userOverridesPath = path
}

// UserOverridesPath returns the configured YAML overrides path.
func (r *ModelRegistry) UserOverridesPath() string {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.userOverridesPath
}

// BaseModelList reparses the unmodified raw model-list snapshot, before user overrides.
func (r *ModelRegistry) BaseModelList() *modeldata.ModelList {
	r.mu.RLock()
	raw := append([]byte(nil), r.modelListRaw...)
	r.mu.RUnlock()
	if len(raw) == 0 {
		return nil
	}
	...
	if err != nil {
		slog.Warn("failed to parse base model list", "error", err)
		return nil
	}
	return list
}

// ReloadUserOverrides reapplies the current user override YAML over the base model list.
func (r *ModelRegistry) ReloadUserOverrides() error {
	r.mu.RLock()
	raw := append([]byte(nil), r.modelListRaw...)
	r.mu.RUnlock()
	if len(raw) == 0 {
	...
	list, err := modeldata.Parse(raw)
	if err != nil {
		return fmt.Errorf("parse base model list: %w", err)
	}
	r.applyUserOverridesToList(list)
	r.setModelListAndEnrich(list, raw)
	return nil
}

// applyUserOverridesToList loads the user overrides YAML (if configured) and
// merges it into list in place. Errors loading the file are logged but never
// propagate, since the upstream/local list is still usable on its own.
// Caller does not need to hold any lock.
func (r *ModelRegistry) applyUserOverridesToList(list *modeldata.ModelList) {
	if list == nil {
		return
	}
	r.mu.RLock()
	path := r.userOverridesPath
	r.mu.RUnlock()
	if path == "" {
		return
	}
	...
	if err != nil {
		slog.Warn("failed to load user model overrides", "path", path, "error", err)
		return
	}
	if overrides == nil {
		return
	}
	models, providerModels := modeldata.ApplyUserOverrides(list, overrides)
	if models > 0 || providerModels > 0 {
		slog.Info("applied user model overrides",
			"path", path,
			"models", models,
