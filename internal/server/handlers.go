...
type Handler struct {
	...
	anthropicIngressSvc     *anthropicIngressService
	anthropicIngressSvcOnce sync.Once
}
...
func (h *Handler) anthropicIngress() *anthropicIngressService {
	h.anthropicIngressSvcOnce.Do(func() {
		s := newAnthropicIngressService(
			h.provider,
			h.modelResolver,
			h.modelAuthorizer,
			h.workflowPolicyResolver,
			h.fallbackResolver,
			h.translatedRequestPatcher,
			h.logger,
			h.usageLogger,
		...
		h.anthropicIngressSvc = s
	})
	return h.anthropicIngressSvc
}

func (h *Handler) AnthropicMessages(c *echo.Context) error {
	return h.anthropicIngress().Messages(c)
}

func (h *Handler) AnthropicCountTokens(c *echo.Context) error {
	return h.anthropicIngress().CountTokens(c)
}
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      core.RerankRequest  true  "Rerank request"
// @Success      200      {object}  core.RerankResponse
// @Failure      400      {object}  core.OpenAIErrorEnvelope
// @Failure      401      {object}  core.OpenAIErrorEnvelope
// @Failure      429      {object}  core.OpenAIErrorEnvelope
// @Failure      501      {object}  core.OpenAIErrorEnvelope
...
func (h *Handler) Rerank(c *echo.Context) error {
	return h.translatedInference().Rerank(c)
}
