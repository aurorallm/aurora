...
import (
	...
	provideradapter "aurora/internal/providers"
...
func (l *capturingAuditLogger) BroadcastLive(*auditlog.LogEntry) {}

func (l *capturingAuditLogger) SubscribeLive(_ int) *auditlog.LogSubscriber {
	ch := make(chan *auditlog.LogEntry, 1)
	close(ch)
	return &auditlog.LogSubscriber{ID: "capture", Entries: ch}
}

func (l *capturingAuditLogger) UnsubscribeLive(_ string) {}

func (l *capturingAuditLogger) LiveSubscriberCount() int { return 0 }
...
func TestResponsesLifecycle_ReturnsSuccessWhenSnapshotStoreFails(t *testing.T) {
	...
	counter := telemetry.ResponseSnapshotStoreFailures.WithLabelValues("mock", "", "store")
...
func (m *mockPricingResolver) ResolvePricing(_, _ string) *usage.PricingResult {
	if m.pricing == nil {
		return nil
	}
	return &usage.PricingResult{Pricing: m.pricing}
}
