...
func (s *Server) Start(ctx context.Context, addr string) error {
	// In bench mode (AURORA_MINIMAL_BENCH_MODE=true), use the optimized listener
	// (SO_CONDITIONAL_ACCEPT on Windows) to prevent TCP RST storms under high
	// connection rates, and bind the listener through echo's StartConfig so that
	// h2c (configured in BeforeServeFunc) is set before Serve is called.
	if os.Getenv("AURORA_MINIMAL_BENCH_MODE") == "true" {
...
func configureGatewayHTTPServer(server *http.Server) error {
	...
	if isBenchMode {
		...
		// Enable h2c (cleartext HTTP/2) for benchmark mode so oha, h2load,
		// and other h2c-capable tools can multiplex requests over a single
		// connection instead of opening one TCP connection per request.
		server.Protocols = new(http.Protocols)
		server.Protocols.SetHTTP1(true)
		server.Protocols.SetUnencryptedHTTP2(true)
	}

	// Also enable h2c when explicitly requested outside bench mode.
