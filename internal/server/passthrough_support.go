...
func (s *passthroughService) logPassthroughUsage(c *echo.Context, body []byte, providerType, providerName, endpoint string, info *core.PassthroughRouteInfo) {
	...
	if s.pricingResolver != nil {
		if pricing := s.pricingResolver.ResolvePricing(model, providerType); pricing != nil {
			costResult := usage.CalculateUsageCost(entry.InputTokens, entry.OutputTokens, entry.RawData, providerType, pricing)
