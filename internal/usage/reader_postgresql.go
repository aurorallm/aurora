...
func (r *PostgreSQLReader) GetSummary(ctx context.Context, params UsageQueryParams) (*UsageSummary, error) {
	...
	promptCacheCols := pgPromptCacheSummaryExpr()
...
func pgPromptCacheSummaryExpr() string {
	return `, COALESCE(SUM(
		COALESCE((raw_data->>'cache_read_input_tokens')::bigint, 0) +
		COALESCE((raw_data->>'prompt_cached_tokens')::bigint, 0) +
		COALESCE((raw_data->>'cached_tokens')::bigint, 0)
	), 0),
	COALESCE(SUM(COALESCE((raw_data->>'cache_creation_input_tokens')::bigint, 0)), 0)`
}
