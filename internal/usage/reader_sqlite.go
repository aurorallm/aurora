...
func (r *SQLiteReader) GetSummary(ctx context.Context, params UsageQueryParams) (*UsageSummary, error) {
	...
	promptCacheCols := sqlitePromptCacheSummaryExpr()
	query := `SELECT COUNT(*), COALESCE(SUM(input_tokens), 0), COALESCE(SUM(output_tokens), 0), COALESCE(SUM(total_tokens), 0)` + promptCacheCols + costCols + `
			FROM usage` + where

	summary := &UsageSummary{}
	err = r.db.QueryRowContext(ctx, query, args...).Scan(
		&summary.TotalRequests, &summary.TotalInput, &summary.TotalOutput, &summary.TotalTokens,
		&summary.TotalCachedInputTokens, &summary.TotalCacheWriteInputTokens,
...
func sqlitePromptCacheSummaryExpr() string {
	return `, COALESCE(SUM(
		COALESCE(CAST(json_extract(raw_data, '$.cache_read_input_tokens') AS INTEGER), 0) +
		COALESCE(CAST(json_extract(raw_data, '$.prompt_cached_tokens') AS INTEGER), 0) +
		COALESCE(CAST(json_extract(raw_data, '$.cached_tokens') AS INTEGER), 0)
	), 0),
	COALESCE(SUM(COALESCE(CAST(json_extract(raw_data, '$.cache_creation_input_tokens') AS INTEGER), 0)), 0)`
}
