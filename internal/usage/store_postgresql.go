...
func NewPostgreSQLStore(pool *pgxpool.Pool, retentionDays int) (*PostgreSQLStore, error) {
	...
	costMigrations := []string{
		...
		"ALTER TABLE usage ADD COLUMN IF NOT EXISTS tenant_id TEXT",
	...
	if _, err := pool.Exec(ctx, `
		...
		SET
			tenant_id = CASE
				...
				THEN split_part(substr(user_path, length('/_tenants/') + 1), '/', 1)
				ELSE tenant_id
			END,
			user_path = CASE
				WHEN strpos(substr(user_path, length('/_tenants/') + 1), '/') = 0 THEN '/'
				ELSE '/' || substr(substr(user_path, length('/_tenants/') + 1), strpos(substr(user_path, length('/_tenants/') + 1), '/') + 1)
			END
		WHERE user_path LIKE '/\_tenants/%' ESCAPE '\'
	`); err != nil {
