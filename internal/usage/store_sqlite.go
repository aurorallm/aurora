...
const (
	...
	maxEntriesPerBatch   = maxSQLiteParams / columnsPerUsageEntry // 49 entries
)
...
func NewSQLiteStore(db *sql.DB, retentionDays int) (*SQLiteStore, error) {
	...
	costMigrations := []string{
		...
		"ALTER TABLE usage ADD COLUMN tenant_id TEXT",
	...
	if _, err := db.Exec(`
		...
		SET
			tenant_id = CASE
				WHEN tenant_id IS NULL OR TRIM(tenant_id) = '' OR TRIM(tenant_id) = 'default'
				THEN substr(substr(user_path, length('/_tenants/') + 1), 1, instr(substr(user_path, length('/_tenants/') + 1) || '/', '/') - 1)
				ELSE tenant_id
			END,
			user_path = CASE
				WHEN instr(substr(user_path, length('/_tenants/') + 1), '/') = 0 THEN '/'
				ELSE '/' || substr(substr(user_path, length('/_tenants/') + 1), instr(substr(user_path, length('/_tenants/') + 1), '/') + 1)
			END
		WHERE user_path LIKE '/_tenants/%'
	`); err != nil {
		return nil, fmt.Errorf("failed to migrate legacy tenant usage paths: %w", err)
	}
	...
	indexes := []string{
		...
		"CREATE INDEX IF NOT EXISTS idx_usage_tenant_user_path ON usage(tenant_id, user_path)",
...
func (s *SQLiteStore) WriteBatch(ctx context.Context, entries []*UsageEntry) error {
	...
	for i := 0; i < len(entries); i += maxEntriesPerBatch {
		...
		query := `INSERT OR IGNORE INTO usage (id, request_id, provider_id, timestamp, model, provider, provider_name,
			endpoint, tenant_id, user_path, cache_type, input_tokens, output_tokens, total_tokens, raw_data,
